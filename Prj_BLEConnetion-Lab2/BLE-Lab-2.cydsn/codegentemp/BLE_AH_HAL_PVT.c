...
{
    /* Setup ISR */
    BLE_AH_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

    BLE_AH_HAL_Uart_Start();
}
...
void CyBLE_Uart_Stop (void)
{
    ...
    BLE_AH_uart_isr_Stop();

    BLE_AH_HAL_Uart_Stop();
}
...
{
    BLE_AH_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
}
...
/* Interface to BLE_AH_bless_isr_Enable function */
void CyBleHal_EnableGlobalInterrupts(void)
{
    BLE_AH_bless_isr_Enable();
}

/* Interface to BLE_AH_bless_isr_Disable function */
...
void CyBle_HalInit(void)
{
    ...
    BLE_AH_bless_isr_SetVector(&CyBLE_Bless_Interrupt);
...
{
    BLE_AH_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
